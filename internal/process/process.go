...
import (
	...
	"github.com/hugeleaf/supervisord/internal/config"
	"github.com/hugeleaf/supervisord/internal/events"
	"github.com/hugeleaf/supervisord/internal/logger"
	"github.com/hugeleaf/supervisord/internal/signals"
...
func (p *Process) waitForExit(startSecs int64) {
	...
	// If startsecs is 0, the finishCb is called immediately after starting,
	// so we need to update the state here when the program exits
	if startSecs <= 0 && p.state == Running {
		exitCode, err := p.getExitCode()
		// Check if exit code is in expected exit codes (normal exit)
		if err == nil && p.inExitCodes(exitCode) {
			p.changeStateTo(Exited)
			log.WithFields(log.Fields{"program": p.GetName(), "exitCode": exitCode}).Info("program exited normally")
		} else {
			// Unexpected exit code - treat as fatal
			p.changeStateTo(Fatal)
			log.WithFields(log.Fields{"program": p.GetName(), "exitCode": exitCode}).Info("program exited unexpectedly")
		}
	}
}
