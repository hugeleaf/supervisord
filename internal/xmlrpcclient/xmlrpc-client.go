...
// SetPathPrefix set the path prefix for HTTP requests
func (r *XMLRPCClient) SetPathPrefix(prefix string) {
	r.pathPrefix = prefix
}

// URL return the RPC url
func (r *XMLRPCClient) URL() string {
	return fmt.Sprintf("%s%s/RPC2", r.serverurl, r.pathPrefix)
}
...
func (r *XMLRPCClient) processResponse(resp *http.Response, processBody func(io.ReadCloser, error)) {
	...
	if resp.StatusCode/100 != 2 {
		if resp.StatusCode == 401 {
			fmt.Fprintf(os.Stderr, "Authentication failed, Please check your username and password.\n")
			fmt.Fprintf(os.Stderr, "You can provide credentials via:\n")
			fmt.Fprintf(os.Stderr, "  1. Configure them in [supervisorctl] section of supervisord.conf\n")
			fmt.Fprintf(os.Stderr, "  2. Use command line options: -u <username> -P <password>\n")
		} else if r.verbose {
...
func (r *XMLRPCClient) postUnixHTTP(method string, path string, data interface{}, processBody func(io.ReadCloser, error)) {
	...
	req, err := r.createHTTPRequest(method, r.pathPrefix + "/RPC2", data)
