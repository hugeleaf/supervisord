...
// GetPathPrefix get the path prefix from inet_http_server or unix_http_server configuration
// The path prefix is used when the supervisord is behind a reverse proxy
func (c *Config) GetPathPrefix() string {
	...
	if ok {
		prefix := entry.GetString("path_prefix", "")
