...
import (
	...
	"github.com/hugeleaf/supervisord/internal/types"
)
...
func NewSupervisorRestful(supervisor *Supervisor, pathPrefix string) *SupervisorRestful {
	return &SupervisorRestful{router: mux.NewRouter(), supervisor: supervisor, pathPrefix: pathPrefix}
}
...
func (sr *SupervisorRestful) CreateProgramHandler() http.Handler {
	sr.router.HandleFunc(sr.pathPrefix+"/program/list", sr.ListProgram).Methods("GET")
	sr.router.HandleFunc(sr.pathPrefix+"/program/start/{name}", sr.StartProgram).Methods("POST", "PUT")
	sr.router.HandleFunc(sr.pathPrefix+"/program/stop/{name}", sr.StopProgram).Methods("POST", "PUT")
	sr.router.HandleFunc(sr.pathPrefix+"/program/log/{name}/stdout", sr.ReadStdoutLog).Methods("GET")
	sr.router.HandleFunc(sr.pathPrefix+"/program/startPrograms", sr.StartPrograms).Methods("POST", "PUT")
	sr.router.HandleFunc(sr.pathPrefix+"/program/stopPrograms", sr.StopPrograms).Methods("POST", "PUT")
	return sr.router
}
...
func (sr *SupervisorRestful) CreateSupervisorHandler() http.Handler {
	sr.router.HandleFunc(sr.pathPrefix+"/supervisor/shutdown", sr.Shutdown).Methods("PUT", "POST")
	sr.router.HandleFunc(sr.pathPrefix+"/supervisor/reload", sr.Reload).Methods("PUT", "POST")
	return sr.router
}
