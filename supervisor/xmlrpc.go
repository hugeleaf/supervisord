...
import (
	...
	"github.com/hugeleaf/supervisord/internal/process"
...
func (p *XMLRPC) StartUnixHTTPServer(user string, password string, listenAddr string, s *Supervisor, pathPrefix string, startedCb func()) {
	os.Remove(listenAddr)
	p.startHTTPServer(user, password, "unix", listenAddr, s, pathPrefix, startedCb)
}

// StartInetHTTPServer start http server on tcp with path listenAddr. If both user and password are not empty, the user
// must provide user and password for basic authentication when making a XML RPC request.
func (p *XMLRPC) StartInetHTTPServer(user string, password string, listenAddr string, s *Supervisor, pathPrefix string, startedCb func()) {
	p.startHTTPServer(user, password, "tcp", listenAddr, s, pathPrefix, startedCb)
}
...
func (p *XMLRPC) startHTTPServer(user string, password string, protocol string, listenAddr string, s *Supervisor, pathPrefix string, startedCb func()) {
	...
	// Register all handlers with path prefix
	// Note: http.ServeMux matches longest prefix first, so order doesn't matter
	// But we need to ensure / doesn't match everything by using a custom handler

	mux.Handle(pathPrefix+"/RPC2", newHTTPBasicAuth(user, password, p.createRPCServer(s)))

	progRestHandler := NewSupervisorRestful(s, pathPrefix).CreateProgramHandler()
	mux.Handle(pathPrefix+"/program/", newHTTPBasicAuth(user, password, progRestHandler))

	supervisorRestHandler := NewSupervisorRestful(s, pathPrefix).CreateSupervisorHandler()
	mux.Handle(pathPrefix+"/supervisor/", newHTTPBasicAuth(user, password, supervisorRestHandler))

	logtailHandler := NewLogtail(s, pathPrefix).CreateHandler()
	mux.Handle(pathPrefix+"/logtail/", newHTTPBasicAuth(user, password, logtailHandler))

	mux.Handle(pathPrefix+"/metrics", promhttp.Handler())

	// WebGUI handler - handles /webgui/* and redirects / to /webgui/
	webguiHandler := NewSupervisorWebgui(s, pathPrefix).CreateHandler()
	mux.Handle(pathPrefix+"/", newHTTPBasicAuth(user, password, webguiHandler))

	listener, err := net.Listen(protocol, listenAddr)
	if err == nil {
		log.WithFields(log.Fields{"addr": listenAddr, "protocol": protocol, "path_prefix": pathPrefix}).Info("success to listen on address")
