...
import (
	...
	"github.com/hugeleaf/supervisord/internal/xmlrpcclient"
...
func (x *CtlCommand) getPathPrefix() string {
	options.Configuration, _ = findSupervisordConf()
	if _, err := os.Stat(options.Configuration); err == nil {
		myconfig := config.NewConfig(options.Configuration)
		myconfig.Load()
		return myconfig.GetPathPrefix()
	}
	return ""
}
...
func (x *CtlCommand) createRPCClient() *xmlrpcclient.XMLRPCClient {
	...
	rpcc.SetPathPrefix(x.getPathPrefix())
	return rpcc
}
