...
import (
	...
	logger "github.com/hugeleaf/supervisord/internal/logger"
)
...
func NewLogtail(supervisor *Supervisor, pathPrefix string) *Logtail {
	return &Logtail{router: mux.NewRouter(), supervisor: supervisor, pathPrefix: pathPrefix}
}
...
func (lt *Logtail) CreateHandler() http.Handler {
	lt.router.HandleFunc(lt.pathPrefix+"/logtail/{program}/stdout", lt.getStdoutLog).Methods("GET")
	lt.router.HandleFunc(lt.pathPrefix+"/logtail/{program}/stderr", lt.getStderrLog).Methods("GET")
	return lt.router
}
